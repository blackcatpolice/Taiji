...
import { Category, CategoryClient } from '@/apis/API';
...
const category: React.FC<{}> = ({ }) => {
    ...
    var categoryClient = new CategoryClient();

    const handleCreateCategory = async (data: any) => {
        return await categoryClient.postAddOrUpdate(data)
    }
    const getCategoryAddOrUpdate = async (id?: any) => {
        const res = await categoryClient.getAddOrUpdate(id)
        handleUpdateFormData({ data: res.data.modelData, modelType: res.data.modelType })
    }
    const handleDeleteCategory = async (data: any) => {
        await categoryClient.delete(data.id)
    }
        ...
        <PageContainer>
            <ProTable<Category>
                ...
                request={async (params, sorter, filter) => {
                    var res = await categoryClient.list(params.current, params.pageSize)
